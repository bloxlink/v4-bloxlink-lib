...
class TestPydanticList:
    ...
    def test_pydantic_list_length(self, input_list, expected_length):
        ...
        assert len(test_list) == expected_length, f"PydanticList should have {
            expected_length} items."

    @pytest.mark.parametrize("input_list, value_to_add, expected_length", [
        ([1, 2, 3], 4, 4),
    ])
    ...
    @pytest.mark.parametrize("input_list, expected_str", [
        ([1, 2, 3], "[1, 2, 3]"),
    ])
    def test_pydantic_list_str(self, input_list, expected_str):
        ...
        assert str(
            test_list) == expected_str, f"PydanticList should return a string of the items."

    @pytest.mark.parametrize("input_list, value_to_check", [
        ([1, 2, 3], 2),
    ])
    def test_pydantic_list_contains(self, input_list, value_to_check):
        ...
        assert value_to_check in test_list, f"PydanticList should contain {
            value_to_check}."

    @pytest.mark.parametrize("input_list, value_to_remove, expected_length", [
        ([1, 2, 3], 2, 2),
    ])
    ...
    @pytest.mark.parametrize("input_list, value_to_pop, expected_length", [
        ([1, 2, 3], 1, 2),
    ])
    ...
    @pytest.mark.parametrize("input_list, values_to_extend, expected_length", [
        ([1, 2, 3], [4, 5], 5),
    ])
    ...
    def test_pydantic_list_iter(self, input_list, expected_list):
        ...
        assert list(
            test_list) == expected_list, "PydanticList should iterate correctly."
