...
class TestPydanticDict:
    ...
    @pytest.mark.parametrize("input_dict, expected_length", [
        ({"a": 1, "b": 2, "c": 3}, 3),
    ])
    def test_pydantic_dict_length(self, input_dict, expected_length):
        ...
        assert len(test_dict) == expected_length, f"PydanticDict should have {
            expected_length} items."

    @pytest.mark.parametrize("input_dict, key_to_add, value_to_add, expected_length", [
        ({"a": 1, "b": 2, "c": 3}, "d", 4, 4),
    ])
    def test_pydantic_dict_add(self, input_dict, key_to_add, value_to_add, expected_length):
    ...
    @pytest.mark.parametrize("input_dict, expected_str", [
    ...
    def test_pydantic_dict_str(self, input_dict, expected_str):
        ...
        assert str(
            test_dict) == expected_str, f"PydanticDict should return a string of the items."

    @pytest.mark.parametrize("input_dict, key_to_check", [
        ({"a": 1, "b": 2, "c": 3}, "b"),
    ])
    def test_pydantic_dict_contains(self, input_dict, key_to_check):
        ...
        assert key_to_check in test_dict, f"PydanticDict should contain {
            key_to_check}."

    @pytest.mark.parametrize("input_dict, key_to_remove, expected_length", [
        ({"a": 1, "b": 2, "c": 3}, "b", 2),
    ])
    ...
    @pytest.mark.parametrize("input_dict, key_to_discard, expected_length", [
        ({"a": 1, "b": 2, "c": 3}, "b", 2),
    ])
    ...
    @pytest.mark.parametrize("input_dict, items_to_update, expected_length", [
    ...
    @pytest.mark.parametrize("input_dict, expected_list", [
        ({"a": 1, "b": 2, "c": 3}, [("a", 1), ("b", 2), ("c", 3)]),
    ])
    def test_pydantic_dict_iter(self, input_dict, expected_list):
        ...
        assert list(test_dict.items()
                    ) == expected_list, "PydanticDict should iterate correctly."
